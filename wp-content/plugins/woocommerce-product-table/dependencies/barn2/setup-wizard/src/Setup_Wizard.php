 ...
 */
namespace Barn2\Plugin\WC_Product_Table\Dependencies\Setup_Wizard;

use Barn2\Plugin\WC_Product_Table\Dependencies\Setup_Wizard\Interfaces\Bootable;
...
class Setup_Wizard implements Bootable, JsonSerializable
{
    ...
    public function register_admin_page()
    {
        ...
        $page_title = \sprintf(__('%s setup wizard', 'woocommerce-product-table'), $this->plugin->get_name());
    ...
    public function load_wizard_restart_assets()
    {
		...
		<script>
			jQuery( '.barn2-wiz-restart-btn' ).on( 'click', function( e ) {
				/* translators: %s: The name of the plugin. */
				return confirm( '<?php 
        echo \esc_html(\sprintf(__('Warning: This will overwrite your existing settings for %s. Are you sure you want to continue?', 'woocommerce-product-table'), $this->plugin->get_name()));
        ?>' );
			});
		</script>
		<?php 
    ...
    public function add_restart_btn()
    {
		...
		<div class="barn2-setup-wizard-restart">
			<hr>
			<h3><?php 
        esc_html_e('Setup wizard', 'woocommerce-product-table');
        ?></h3>
			<p><?php 
        esc_html_e('If you need to access the setup wizard again, please click on the button below.', 'woocommerce-product-table');
        ?></p>
			<a href="<?php 
        echo \esc_url($url);
    ...
    public function add_restart_link(string $wc_section_id, string $title_option_id)
    {
        \add_filter("woocommerce_get_settings_{$wc_section_id}", function ($settings) use($title_option_id) {
            ...
            if ($title_setting && isset($title_setting[\key($title_setting)]['desc'])) {
                ...
                $new_desc = \substr_replace($desc, ' | <a class="barn2-wiz-restart-btn" href="' . \esc_url($url) . '">' . esc_html__('Setup wizard', 'woocommerce-product-table') . '</a>', $p_closing_tag, 0);
