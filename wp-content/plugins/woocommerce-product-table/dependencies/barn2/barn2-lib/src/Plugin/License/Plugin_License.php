...
namespace Barn2\Plugin\WC_Product_Table\Dependencies\Lib\Plugin\License;
...
class Plugin_License implements Registerable, License
{
    ...
    public function get_error_message()
    {
        ...
        switch ($this->get_error_code()) {
            ...
            case 'no_activations_left':
                ...
                $read_more_link = Util::format_store_link('kb/license-key-problems', __('Read more', 'woocommerce-product-table'));
            ...
            case 'expired':
                ...
                $renewal_link = Util::format_link($this->get_renewal_url(), __('Renew now for 20% discount.', 'woocommerce-product-table'), \true);
                /* translators: support for RTL, 1: the license error, 2: a link */
    ...
    {
        return __('Your license key is not active. Please reactivate or save the settings.', 'woocommerce-product-table');
    }
    private function get_license_expired_message()
    {
        return __('Your license key has expired.', 'woocommerce-product-table');
    }
    private function get_license_disabled_message()
    {
        return \sprintf(
            /* translators: 1: purchase link start, 2: purchase link end. */
            __('Your license key has been disabled. Please %1$spurchase a new license key%2$s to continue using the plugin.', 'woocommerce-product-table'),
