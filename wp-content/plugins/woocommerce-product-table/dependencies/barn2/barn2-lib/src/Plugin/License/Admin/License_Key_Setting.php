...
namespace Barn2\Plugin\WC_Product_Table\Dependencies\Lib\Plugin\License\Admin;

use Barn2\Plugin\WC_Product_Table\Dependencies\Lib\Admin\Settings_API_Helper;
use Barn2\Plugin\WC_Product_Table\Dependencies\Lib\Plugin\License\License;
...
class License_Key_Setting implements Registerable, License_Setting
{
    ...
    public function process_license_action()
    {
        if ($this->is_license_action(self::ACTIVATE_KEY)) {
            ...
            if (isset($license_setting['license'])) {
                ...
                $this->add_settings_message(__('License key successfully activated.', 'woocommerce-product-table'), __('There was an error activating your license key.', 'woocommerce-product-table'), $activated);
            }
        } elseif ($this->is_license_action(self::DEACTIVATE_KEY)) {
            $deactivated = $this->license->deactivate();
            $this->add_settings_message(__('License key successfully deactivated.', 'woocommerce-product-table'), __('There was an error deactivating your license key, please try again.', 'woocommerce-product-table'), $deactivated);
        } elseif ($this->is_license_action(self::CHECK_KEY)) {
            $this->license->refresh();
            $this->add_settings_message(__('The license key looks good!', 'woocommerce-product-table'), __('There\'s a problem with your license key.', 'woocommerce-product-table'), $this->license->is_active());
        }
    }
    ...
    {
        $setting = ['title' => __('License key', 'woocommerce-product-table'), 'type' => 'text', 'id' => $this->get_license_setting_name() . '[license]', 'desc' => $this->get_license_description(), 'class' => 'regular-text'];
        if ($this->is_woocommerce) {
            ...
            $setting['title'] = __('License Key', 'woocommerce-product-table');
    ...
    {
        $buttons = ['check' => $this->license_action_button(self::CHECK_KEY, __('Check', 'woocommerce-product-table')), 'activate' => $this->license_action_button(self::ACTIVATE_KEY, __('Activate', 'woocommerce-product-table')), 'deactivate' => $this->license_action_button(self::DEACTIVATE_KEY, __('Deactivate', 'woocommerce-product-table'))];
