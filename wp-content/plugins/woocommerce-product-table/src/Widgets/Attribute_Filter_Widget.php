 ...
 */
class Attribute_Filter_Widget extends Product_Table_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = __( 'Display a list of attributes to filter products in your product table.', 'woocommerce-product-table' );
		$this->widget_id          = 'woocommerce_pt_layered_nav';
		$this->widget_name        = __( 'Product Table: Filter by Attribute', 'woocommerce-product-table' );
	...
	public function init_settings() {
		...
		$this->settings = [
			'title'        => [
				'type'  => 'text',
				'std'   => __( 'Filter by', 'woocommerce-product-table' ),
				'label' => __( 'Title', 'woocommerce-product-table' ),
			],
			'attribute'    => [
				'type'    => 'select',
				'std'     => $std_attribute,
				'label'   => __( 'Attribute', 'woocommerce-product-table' ),
				'options' => $attribute_array,
			],
			'display_type' => [
				'type'    => 'select',
				'std'     => 'list',
				'label'   => __( 'Display type', 'woocommerce-product-table' ),
				'options' => [
					'list'     => __( 'List', 'woocommerce-product-table' ),
					'dropdown' => __( 'Dropdown', 'woocommerce-product-table' ),
				],
			],
			'query_type'   => [
				'type'    => 'select',
				'std'     => 'and',
				'label'   => __( 'Query type', 'woocommerce-product-table' ),
				'options' => [
					'and' => __( 'AND', 'woocommerce-product-table' ),
					'or'  => __( 'OR', 'woocommerce-product-table' ),
				],
			],
		];
	}
	...
	protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
		...
		if ( $taxonomy !== $this->get_current_taxonomy() ) {
			...
			$any_label      = apply_filters( 'woocommerce_layered_nav_any_label', sprintf( __( 'Any %s', 'woocommerce-product-table' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
			...
			if ( $multiple ) {
				echo '<button class="woocommerce-widget-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'woocommerce-product-table' ) . '">' . esc_html__( 'Apply', 'woocommerce-product-table' ) . '</button>';
			}
			...
			echo wc_query_string_form_fields( null, [ 'filter_' . $taxonomy_filter_name, 'query_type_' . $taxonomy_filter_name ], '', true );
			echo '</form>';
				...
				if ( jQuery().selectWoo ) {
					var wc_layered_nav_select = function() {
						jQuery( '.dropdown_layered_nav_" . esc_js( $taxonomy_filter_name ) . "' ).selectWoo( {
							...
							language: {
								noResults: function() {
									return '" . esc_js( _x( 'No matches found', 'enhanced select', 'woocommerce-product-table' ) ) . "';
								}
							}
						} );
					};
