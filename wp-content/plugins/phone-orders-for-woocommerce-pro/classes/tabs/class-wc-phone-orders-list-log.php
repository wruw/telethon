...
class WC_Phone_Orders_List_Log extends WP_List_Table {
	...
	private function update_order_number( &$item ) {
		...
		$item['order_number'] = '<a href="' . $order->get_edit_order_url() . '" target="_blank">' . $item['order_number'] . '</a>';
	}
	...
	function update_customer( &$item ) {
		...
		$customer_login_html = '';
		if ( $customer_id AND current_user_can('list_users') AND current_user_can( 'edit_user',  $customer_id ) ) {
			$customer_url = add_query_arg( array( 'user_id' => $customer_id, ), admin_url( 'user-edit.php' ) );
			$customer_login_html = __( 'Customer login',
					'phone-orders-for-woocommerce' ) . " : " . '<a href="' . $customer_url . '" target="_blank">' . get_userdata( $customer_id )->user_login . '</a><br />';
		};
