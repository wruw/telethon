...
class WC_Phone_Orders_Tabs_Helper {
	...
	public static function add_log( $id, $result, $order_id = 0 ) {
		...
		if ( $count ) {
			...
			$r = $wpdb->query( $wpdb->prepare(
				"INSERT INTO $table_name (ID, time_updated, user_id, user_name, order_id, order_number, customer,customer_id, items, discount, fees, shipping, total ) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s' )"
				, array(
				$id,
				$time,
				$user_id,
	...
	private static function create_row_shipping_method_column( $result ) {
		$row = '';
		if ( ! empty( $result['chosen_shipping_method']['total_html'] ) ) {
			$row .= $result['chosen_shipping_method']['label'] . ' : ' . $result['chosen_shipping_method']['total_html'];
		}
